...
},{"process":18}],16:[function(require,module,exports) {
var bundleURL = null;
...
var monthHeaderHeight = 50;
var lineWidth = 2;
var canvasImageWidth = canvas.get(0).width - lineWidth * 5;
var canvasImageHeight = canvas.get(0).height - headerHeight - monthHeaderHeight * 3 - lineWidth * 3;
var imgw = canvasImageWidth / 4 | 0;
var imgh = canvasImageHeight / 3 | 0;

var canvasBackground = '#6d6d6d';
var textColor = '#f7f7f7';
var titleBackground = '#f0f0f0';
var titleTextColor = '#494949';
var fontfamilies = "游ゴシック体, 'Yu Gothic', YuGothic, 'ヒラギノ角ゴシック Pro', 'Hiragino Kaku Gothic Pro', メイリオ, Meiryo, Osaka, 'ＭＳ Ｐゴシック', 'MS PGothic', sans-serif";
var fontweight = 800;
var fontSizeMonth = 40;
var fontSizeTitle = 80;
var monthTextStyle = fontweight + " " + fontSizeMonth + "px " + fontfamilies;
var titleTextStyle = fontweight + " " + fontSizeTitle + "px " + fontfamilies;
...
function initCanvas(ctx) {
  ...
  Array(12).fill(0).map(function (i, v) {
    var x = calcX(v);
    var y = calcY(v);
    var xPos = lineWidth + imgw / 2 + (imgw + lineWidth) * x;
    var yPos = headerHeight + monthHeaderHeight + (imgh + lineWidth + monthHeaderHeight) * y - fontSizeMonth / 4;
...
function drawText(ctx) {
  ...
  var drawtext = '';
  var name = $('#name-text').get(0).value;
  if (name !== '') {
...
window.onload = function () {
  ...
  var files = Array(12).fill(0).map(function (i, v) {
    var p = $('<p></p>');

    var inputId = "image-" + v;
    var f = $("<input id=\"" + inputId + "\" type=\"file\" />");
    f.change = function () {};

    var l = $("<label for=\"" + inputId + "\">" + (v + 1) + "\u6708</label>");
    ...
    f.on('fileimageloaded', function (event) {
      if (typeof event.currentTarget.files[0] !== 'undefined') {
        var file = event.currentTarget.files[0];
        var img = new Image();
        img.onload = function () {
          var width = file.width = img.width;
          var height = file.height = img.height;
          var x = calcX(v);
          var y = calcY(v);
          //ctx.drawImage(img, 0, 0);
          var xPos = calcBlockX(x);
          var yPos = calcBlockY(y);
          if (width > height) {
            ctx.drawImage(img, (width - height) / 2 | 0, 0, height, height, xPos, yPos, imgw, imgh);
          } else {
            ctx.drawImage(img, 0, (height - width) / 2 | 0, width, width, xPos, yPos, imgw, imgh);
          }
        };
...
},{"jquery":8,"./index.scss":7,"bootstrap":9,"bootstrap-fileinput":11}],0:[function(require,module,exports) {
var global = (1, eval)('this');
...
if (!module.bundle.parent) {
  var ws = new WebSocket('ws://localhost:64166/');
